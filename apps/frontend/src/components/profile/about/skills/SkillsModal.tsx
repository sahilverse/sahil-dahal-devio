...
import { Wrench, X, Loader2, Check } from "lucide-react";
...
export default function SkillsModal({
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="w-[95%] max-h-[90dvh] sm:max-w-[500px] p-0 overflow-hidden border-none shadow-2xl bg-card rounded-xl flex flex-col">
                <DialogHeader className="px-4 py-4 md:px-6 md:py-5 border-b bg-muted/20 shrink-0">
                    <DialogTitle className="text-sm font-bold flex items-center gap-2 tracking-tight uppercase text-primary/80">
                        <Wrench className="w-4 h-4" /> Manage Skills
                    </DialogTitle>
                </DialogHeader>
                ...
                <DialogFooter className="px-4 py-4 border-t flex flex-row items-center justify-end gap-3 shrink-0">
                    <Button
                        variant="brand"
                        onClick={onClose}
